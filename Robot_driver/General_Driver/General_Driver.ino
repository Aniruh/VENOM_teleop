...
Serial.println("OTA Ready");
  ...
  //RoArmM2_singleJointAbsCtrl(2, -0.22, 250, 40);
  //delay(3000);
  
  RoArmM2_singleJointAbsCtrl(4,1.8, 400, 80);
  delay(3000);

  RoArmM2_singleJointAbsCtrl(3, 3.14, 400, 70);
  delay(5000);

  RoArmM2_shoulderJointCtrlRad(1, 0.0, 400, 70);
  delay(3000);
  Serial.println("==== SERVO INDEX DEBUG ====");

int drvIdx = servoIdToIndex(SHOULDER_DRIVING_SERVO_ID);
int drnIdx = servoIdToIndex(SHOULDER_DRIVEN_SERVO_ID);
int auxIdx = servoIdToIndex(SHOULDER_DRIVEN_AUX_SERVO_ID);

Serial.print("DRIVING SERVO ID: ");
Serial.print(SHOULDER_DRIVING_SERVO_ID);
Serial.print(" -> INDEX: ");
Serial.println(drvIdx);

Serial.print("DRIVEN SERVO ID: ");
Serial.print(SHOULDER_DRIVEN_SERVO_ID);
Serial.print(" -> INDEX: ");
Serial.println(drnIdx);

Serial.print("AUX SERVO ID: ");
Serial.print(SHOULDER_DRIVEN_AUX_SERVO_ID);
Serial.print(" -> INDEX: ");
Serial.println(auxIdx);

Serial.println("---- GOALPOS VALUES ----");

Serial.print("goalPos[");
Serial.print(drvIdx);
Serial.print("] = ");
Serial.println(goalPos[drvIdx]);

Serial.print("goalPos[");
Serial.print(drnIdx);
Serial.print("] = ");
Serial.println(goalPos[drnIdx]);

Serial.print("goalPos[");
Serial.print(auxIdx);
Serial.print("] = ");
Serial.println(goalPos[auxIdx]);
...
void loop() {
  ...
  //Serial.print("mainType=");
  //Serial.println(mainType);
  //Serial.print("Motion state is : ");
  //Serial.println(motionState);
  Serial.print("yaw pid is  : ");
  Serial.println(yawPID);
  Serial.print("input Left  : ");
  Serial.print(inputLeftOld);
  Serial.print("input Right  : ");
  Serial.println(inputRightOld);
  ...
  if(yawPID)
    {
      ...
      float correctionLeft = error * 0.05;//headingPID.compute(error)
      float correctionRight= error * 0.05;

        Serial.print("correction is  : ");
        Serial.println(error);
      correctedLeft  = inputLeftOld  - correctionLeft;
      correctedRight = inputRightOld + correctionRight;

      
      setpointA = correctedLeft  * spd_rate_A * 1.0;
      setpointB = correctedRight * spd_rate_B * 1.0;

      if (setpointA != setpointA_buffer) {
      pidA.Setpoint(setpointA);
      setpointA_buffer = inputLeftOld;
    }
    
    if (setpointB != setpointB_buffer) {
      pidB.Setpoint(setpointB);
      setpointB_buffer = inputRightOld;
    }
    
    }


//Serial.print("usePIDCompute=");
//Serial.println(usePIDCompute);
