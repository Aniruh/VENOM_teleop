#include "BNO055.h"
//for having a state machine 

bool yawPID = false;

extern float targetHeading;
float targetHeading = 0.00f;
extern float inputLeftOld;
extern float inputRightOld;

float inputLeftOld = 0.00f;
float inputRightOld = 0.00f;
...
//robot initialised at stop state
...
void setGoalSpeed(float inputLeft, float inputRight) {
  ...
  //inputLeft *= 0.6;
  inputLeftOld = inputLeft;
  inputRightOld = inputRight;
  
  if (mainType == 3) {
    //Serial.print("Set goal speed triggered!");
    ...
    */
    ...
    if(inputLeft>=0.10 && inputRight>=0.40)
    {
      //we are moving forward
      motionState = FORWARD;
      //yawPID = true;
      //Serial.println("YAW PID TRUE!");
    } 

     if(inputLeft<0 && inputRight<0.50)
    {
      //we are moving forward
     ...
     if(inputLeft==0.00 && inputRight==0.00)
    {
      //we are moving forward
      motionState = STOPPED;
      yawPID = false;
    ...
    if(motionState == FORWARD && yawPID==false)
    {
      yawPID=true;
      Serial.print(yawPID);
      Serial.print(" @ ");
    ...
    if(inputRight < -2.0 || inputRight > 2.0){
      ...
      setpointA = inputLeft*spd_rate_A*1.0;
      setpointB = inputRight*spd_rate_B*1.0;
